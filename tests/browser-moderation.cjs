...
(async () => {
  ...
  try {
    ...
    const shot = async name => {
      ...
      assert.match(await page.title(), /Moderation|Remove ban/);
    ...
    for (const width of [1440, 390, 320]) {
      ...
      await page.getByRole("tab", { name: "Users", exact: true }).click(); await settle();
      await page.getByRole("button", { name: "Ban user " + email, exact: true }).click();
      const dialog = page.locator("#admin-table-dialog");
      await dialog.getByLabel("User links", { exact: true }).check();
      ...
      await row.getByRole("link", { name: "Remove ban", exact: true }).click(); await settle();
      await shot("confirm-" + width);
      await page.getByRole("button", { name: "Remove ban", exact: true }).focus();
      ...
      await page.getByLabel("Banned entries", { exact: true }).selectOption("link");
      await page.getByRole("button", { name: "Filter", exact: true }).click(); await settle();
      await shot("links-" + width);
      const bannedLink = page.locator(".moderation-list li").filter({ hasText: link.address });
      await bannedLink.getByRole("link", { name: "Remove ban", exact: true }).click(); await settle();
      await page.getByRole("button", { name: "Remove ban", exact: true }).click(); await settle();
