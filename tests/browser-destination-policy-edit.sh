#!/bin/sh
set -eu
image=${1:?usage: browser-destination-policy-edit.sh IMAGE}
node=${NODE_BINARY:-node}
port=${KUTT_BROWSER_PORT:-31134}
case "$port" in ''|*[!0-9]*) exit 2;; esac
[ "$port" -ge 1024 ] && [ "$port" -le 65535 ]
root=$(CDPATH= cd -- "$(dirname "$0")/.." && pwd)
evidence=${KUTT_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/kutt-policy-edit-browser.XXXXXX")}
cid=
...
cid=$(docker run -d --read-only --cap-drop ALL --security-opt no-new-privileges:true --tmpfs /tmp:mode=1777 \
  ...
  -e DB_CLIENT=better-sqlite3 -e DB_FILENAME=/tmp/kutt-policy-edit.sqlite -e REDIS_ENABLED=false \
  ...
  -e KUTT_BROWSER_DISPOSABLE=1 -e CSP_MODE=enforce -e 'DESTINATION_ALLOWED_HOSTS=["192.0.2.1"]' \
  --entrypoint sh "$image" -c 'export JWT_SECRET=$(node -e "console.log(require(\"crypto\").randomBytes(48).toString(\"hex\"))"); exec node tests/destination-policy-browser-fixture.cjs')
attempt=0
...
KUTT_BROWSER_DISPOSABLE=1 KUTT_TEST_URL="http://127.0.0.1:$port" KUTT_EVIDENCE_DIR="$evidence" \
  "$node" "$root/tests/browser-destination-policy-edit.cjs"
