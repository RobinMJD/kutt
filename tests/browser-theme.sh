#!/bin/sh
set -eu
image=${1:?usage: browser-theme.sh IMAGE}
node=${NODE_BINARY:-node}
port=${KUTT_BROWSER_PORT:-31122}
case "$port" in ''|*[!0-9]*) exit 2;; esac
[ "$port" -ge 1024 ] && [ "$port" -le 65535 ]
root=$(CDPATH= cd -- "$(dirname "$0")/.." && pwd)
evidence=${KUTT_EVIDENCE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/kutt-theme-browser.XXXXXX")}
cid=
...
cid=$(docker run -d --read-only --cap-drop ALL --security-opt no-new-privileges:true --tmpfs /tmp:mode=1777 \
  ...
  -e DB_CLIENT=better-sqlite3 -e DB_FILENAME=/tmp/kutt-browser-theme.sqlite -e REDIS_ENABLED=false \
...
KUTT_BROWSER_DISPOSABLE=1 KUTT_TEST_URL="http://127.0.0.1:$port" KUTT_EVIDENCE_DIR="$evidence" \
  "$node" "$root/tests/browser-theme.cjs"
