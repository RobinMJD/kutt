...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 390, 320]) {
      ...
      await submit(form, "POST", "/api/links", "Shorten link");
      await form.getByText("Custom URL is not valid.", { exact: true }).waitFor();
      ...
      for (const adminView of [false, true]) {
        ...
        await submit(edit, "PATCH", endpoint, "Update");
        await edit.getByText("Custom URL is not valid", { exact: true }).waitFor();
        ...
        await submit(edit, "PATCH", endpoint, "Update");
        await edit.getByText("Link has been updated.", { exact: true }).waitFor();
      ...
      await page.getByText("Create shared link", { exact: true }).click();
      const shared = page.getByRole("form", { name: "Create shared link", exact: true });
      await shared.getByLabel("Destination", { exact: true }).fill(target);
      await shared.getByLabel("Alias", { exact: true }).fill(sharedAlias);
      await submit(shared, "POST", sharedPath, "Create link", 303);
      ...
      await sharedEdit.getByLabel("Alias", { exact: true }).fill("browser-" + width + "/shared..pdf");
      await sharedEdit.getByLabel("Description", { exact: true }).fill("Keep shared draft");
      await submit(sharedEdit, "POST", sharedPath, "Save link", 400);
      await page.waitForLoadState("networkidle");
      assert.equal(await sharedEdit.getByLabel("Description", { exact: true }).inputValue(), "Keep shared draft");
      assert(await listing(sharedAlias));
      await capture(width + "-workspace-error");
      const finalAlias = "browser-" + width + "/shared.v2.pdf";
      await sharedEdit.getByLabel("Alias", { exact: true }).fill(finalAlias);
      await submit(sharedEdit, "POST", sharedPath, "Save link", 303);
      await page.waitForLoadState("networkidle");
