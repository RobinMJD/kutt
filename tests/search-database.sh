...
if [ "$engine" = mysql2 ]; then
  ...
  docker run -d --cidfile "$cidfile" --network none --read-only --cap-drop ALL --user mysql \
    ...
    --tmpfs /var/run/mysqld:mode=1777 -e MYSQL_ROOT_PASSWORD=disposable-search-only \
    -e MYSQL_DATABASE=kutt_search_regression -e MYSQL_USER=kutt -e MYSQL_PASSWORD=disposable-search-only \
  ...
  docker run -d --cidfile "$cidfile" --network none --read-only --cap-drop ALL --user postgres \
    ...
    --tmpfs /var/run/postgresql:mode=1777 -e POSTGRES_DB=kutt_search_regression -e POSTGRES_USER=kutt \
    -e POSTGRES_PASSWORD=disposable-search-only -e PGDATA=/var/lib/postgresql/data/fixture \
...
while :; do
  if [ "$engine" = mysql2 ]; then
    if docker exec "$id" mysql -h 127.0.0.1 -u kutt -pdisposable-search-only -e 'SELECT 1' kutt_search_regression >/dev/null 2>&1; then break; fi
  else
    if docker exec "$id" pg_isready -h 127.0.0.1 -U kutt -d kutt_search_regression >/dev/null 2>&1; then break; fi
  fi
  attempt=$((attempt + 1))
