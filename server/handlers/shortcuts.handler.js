...
function endpoint() {
  ...
  if (!url || url.username || url.password || url.host !== host || url.pathname !== "/" || url.search || url.hash) {
    throw new CustomError("Configure a valid HTTPS DEFAULT_DOMAIN before creating a Shortcut.", 503);
  }
...
function recipe(req, res) {
  res.json({ endpoint: endpoint(), method: "POST", header: "X-API-Key",
    ...
    body: { target: "Selected URL", reuse: true }, result: "link",
...
async function create(req, res) {
  ...
  if (!req.body || Array.isArray(req.body) || typeof req.body !== "object" ||
    ...
    throw new CustomError("Only the Shortcut name is accepted. Permissions and expiry are fixed.", 400);
  }
...
function guide(req, res) {
  res.type("text/plain").sendFile(path.join(__dirname, "../../examples/IOS-SHORTCUT.md"));
}
function page(req, res) {
  res.render("shortcuts", { title: "iOS Shortcut", endpoint: endpoint(),
