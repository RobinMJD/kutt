...
const actions = {
  OK: "No action needed.", REDIRECT: "Review the destination redirect; its final page was not checked.",
  ACCESS_RESTRICTED: "Check destination authentication or bot restrictions. No credentials were sent.",
  HEAD_UNSUPPORTED: "The destination does not support HEAD. Verify it manually; no GET fallback is used.",
  HTTP_ERROR: "Check the destination service or update the link after verification.",
  URL_DENIED: "Use public HTTPS on port 443 without credentials or sensitive query parameters for monitoring.",
  ADDRESS_DENIED: "DNS includes a private or reserved address. Correct DNS or leave this destination unmonitored.",
  DNS_FAILED: "Check public A/AAAA resolution and retry after DNS recovery.",
  TIMEOUT: "The destination timed out. Verify its availability and retry later.",
  CONNECTION_FAILED: "Check the destination TLS certificate and network availability.",
  CHECK_FAILED: "Retry later; inspect the application worker if this persists.",
  CONFIGURATION_INVALID: "Destination rules cannot be read. Repair the saved configuration before checking. No request was sent."
};
function strict(body, keys) {
  if (!body || typeof body !== "object" || Array.isArray(body) || Object.keys(body).some(key => !keys.includes(key))) fail("Invalid health-check fields.");
}
function revision(value) { if (!Number.isSafeInteger(value) || value < 0 || value >= Number.MAX_SAFE_INTEGER) fail("A current integer revision is required."); }
...
async function probe(item) {
  ...
  return { name: item.name, code, http_status: status, duration_ms: Math.max(0, Date.now() - started), action: actions[code] };
}
...
async function view(link, user, db = knex) {
  ...
  try {
    ...
    results = results.map(item => ({ name: item.name, code: item.code, duration_ms: item.duration_ms, http_status: item.http_status, action: actions[item.code] }));
  }
...
async function save(req) {
  ...
  if (typeof req.body.enabled !== "boolean" || !Number.isInteger(req.body.interval_hours) || req.body.interval_hours < 1 || req.body.interval_hours > 168) fail("Choose enabled and an interval from 1 to 168 hours.");
  return knex.transaction(async db => {
    ...
    if (Number(row?.revision || 0) !== req.body.revision) fail("Monitoring changed elsewhere. Reload before saving.", 409);
    if (req.body.enabled && (!row?.enabled || row.user_id !== req.user.id)) {
      ...
      if (Number(count.n) >= 100) fail("At most 100 links may be monitored per owner.", 409);
    }
...
async function queue(req) {
  ...
  return knex.transaction(async db => {
    const link = await owned(req, db, true), row = await db("link_health").where({ link_id: link.id }).first(), now = Date.now();
    if (!row?.enabled || Number(row.revision) !== req.body.revision || row.user_id !== req.user.id || Number(row.auth_version) !== Number(req.user.auth_version)) fail("Reload and enable current monitoring before checking.", 409);
    if (row.lease && Number(row.lease_until) > now || row.requested_at && now - Number(row.requested_at) < 60000) fail("A check is running or was requested less than one minute ago.", 429);
...
async function list(req) {
  ...
  if (before !== undefined && (typeof before !== "string" || !/^[1-9]\d{0,14}$/.test(before))) fail("Invalid monitoring cursor.");
